...
class EmployeeStatus extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('employee_status_code', 'uniqueEmployeeStatusCode'),
    ...
    public function uniqueEmployeeStatusCode($attribute, $params) {

        $model = EmployeeStatus::model()->findByAttributes(array('company_id' => $this->company_id, 'employee_status_code' => $this->$attribute));
        if ($model && $model->employee_status_id != $this->employee_status_id) {
            $this->addError($attribute, 'Employee status code selected already taken.');
        }
        return;
    }
...
//        $criteria->compare('employee_status_id', $columns[0]['search']['value'], true);
        $criteria->compare('employee_status_code', $columns[0]['search']['value'], true);
        $criteria->compare('description', $columns[1]['search']['value'], true);
        $criteria->compare('available', $columns[2]['search']['value'], true);
