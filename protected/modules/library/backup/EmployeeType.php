...
class EmployeeType extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('employee_type_code', 'uniqueEmployeeTypeCode'),
    ...
    public function uniqueEmployeeTypeCode($attribute, $params) {

        $model = EmployeeType::model()->findByAttributes(array('company_id' => $this->company_id, 'employee_type_code' => $this->$attribute));
        if ($model && $model->employee_type_id != $this->employee_type_id) {
            $this->addError($attribute, 'Employee type code selected already taken.');
        }
        return;
    }
...
//        $criteria->compare('employee_type_id', $columns[0]['search']['value'], true);
        $criteria->compare('employee_type_code', $columns[0]['search']['value'], true);
