    ...
    <div class="panel-body">
        ...
        <?php if (count($custom_datas) > 0) { ?>
            ...
            foreach ($custom_datas as $key => $val) {
                ...
                $post_name = $val['data_type'] = str_replace(' ', '_', strtolower($val['data_type'])) . "_" . $val['name'] = str_replace(' ', '_', strtolower($val['name']));
                ...
                if ($data_type == 'Text and Numbers') {
                    ...
                    <?php } else { ?>
                        ...
                        <div class="form-group">
                            <label><?php echo $label_name; ?></label>                            
                            <?php echo CHtml::textField($post_name, isset($_POST[$post_name]) ? $_POST[$post_name] : $val['custom_data_value'], array('maxlength' => isset($attr['max_character_length']) ? $attr['max_character_length'] : "", 'class' => 'form-control input-sm', 'placeholder' => $placeholder)); ?>
                ...
                <?php } else if ($data_type == 'Numbers Only') { ?>
                    ...
                    <div class="form-group">
                        ...
                        <?php
                        $value = rtrim(isset($_POST[$post_name]) ? $_POST[$post_name] : $val['custom_data_value'], "0");
                        $value = substr($value, -1) == "." ? rtrim($value, ".") : $value;
                        ?>

                        <?php $decimal_place = $attr['decimal_place']; ?>
                        <?php echo CHtml::numberField($post_name, isset($_POST[$post_name]) ? $_POST[$post_name] : $val['custom_data_value'], array('min' => $attr['min_value'], 'max' => $attr['max_value'], "onkeypress" => "return onlyDotsAndNumbers(this, event, $decimal_place)", 'class' => 'form-control input-sm', 'placeholder' => $placeholder)) ?>
                        <?php echo $form->error($sku_custom_data, $post_name); ?>
                    </div>
