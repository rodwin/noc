...
class InventoryHistory extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('inventory_history_id, company_id, inventory_id, quantity_change, running_total, action, cost_unit, ave_cost_per_unit, destination_zone_id, created_date, created_by, updated_date, updated_by, sort_no', 'safe', 'on' => 'search'),
        );
    }
...
//        $criteria->compare('inventory_history_id', $columns[0]['search']['value']);
    ...
    public function createHistory($company_id, $inventory_id, $transaction_date, $quantity_change, $running_total, $action, $cost_unit = 0, $created_by = null, $zone_id) {
        ...
        $c->order = "sort_no DESC";
        $c->limit = 1;
        $inv_history = InventoryHistory::model()->find($c);
        
        $sort_no = 1;
        if ($inv_history) {
            $sort_no = $inv_history->sort_no + 1;
        }
