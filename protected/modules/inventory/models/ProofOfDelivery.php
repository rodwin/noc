...
class ProofOfDelivery extends CActiveRecord {
    ...
    public function customerData($customer_header_data, $transaction_details, $customer_item_detail_ids) {

        foreach ($transaction_details as $key => $value) {
            $transaction_details[$key] = array_merge($customer_item_detail_ids[$key], $transaction_details[$key]);
        }
